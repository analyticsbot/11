...
sleep 5
# IP addresses for which VPN needs to be active
vpn_ips=(ec2-54-191-109-246.us-west-2.compute.amazonaws.com)
vpn_gw=\$(ip route | grep tun0 | grep 0.0.0.0 | cut -d" " -f3)

for item in \${vpn_ips[*]}
do
echo "Enable \$item to use VPN"
route add -net \$item netmask 255.255.255.255 gw \$vpn_gw dev tun0
done

echo "Disable VPN for all other IPs"
route del -net 0.0.0.0 netmask 128.0.0.0 dev tun0
route del -net 128.0.0.0 netmask 128.0.0.0 dev tun0

echo "Checking new IP address..."
